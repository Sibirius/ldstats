...
function attachEvents() {
  ...
  function updateShares(author, username) {
    var twLink =
      "https://twitter.com/intent/tweet?url=https%3A%2F%2Fldstats.info%2F{{author}}%2F{{username}}&hashtags=ldstats&text=Checkout+my+LudumDare+Stats";
    var fbLink =
      "https://www.facebook.com/sharer/sharer.php?u=https%3A%2F%2Fldstats.info%2F{{author}}%2F{{username}}&t=Checkout+my+LudumDare+Stats";
    var gpLink =
      "https://plus.google.com/share?url=https%3A%2F%2Fldstats.info%2F{{author}}%2F{{username}}";
    ...
    function setLink(at, link) {
      $(at).attr(
        "href",
        link.replace("{{author}}", author).replace("{{username}}", username)
      );
    }
  ...
  if (builtTabs.indexOf(tab) === -1) {
    switch (tab) {
      ...
      case "podium":
        createDataTable(
          "#position-data",
          categories.concat(["total"]),
          window.positions,
          function (value) {
            if (value > 0 && value <= 100) {
              if (value < 4) {
                return (
                  '<td class="highlight top-' +
                  value +
                  '"><div title="TOP ' +
                  value +
                  '"></div></td>'
...
function createDataTable(id, columns, entries, onCellValue, sort) {
  ...
  var _cols = "";
  _.times(columns.length + 2, function (i) {
    _cols += "<colgroup/>";
  });
  ...
  $table
    ...
    .on("click", "th", function (e) {
      ...
      $table.off();
      createDataTable(id, columns, entries, onCellValue, {
        dir: dir,
        col: col,
      });
    });
...
function buildCard() {
  ...
  url += `?ld=${ld}`;
  url += window.run_author ? `&user=${window.run_author}` : "";
  url += window.run_username ? `&username=${window.run_username}` : "";

  var html =
    '<iframe src="' +
    url +
    '" width="100%" height="520" frameborder="0" allowtransparency="true" title="Ludum Dare CARD"></iframe>';
  $("#card-ctn").html(html);
  $("#sharer").val(html);
}
